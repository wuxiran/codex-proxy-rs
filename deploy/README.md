...
观澜自动复活由账号编辑页逐账号授权，现有账号不会因升级自动勾选。后台每 60 秒检查已勾选且启用调度的
失效 OAuth 账号；只有具备匹配签名原件时才提交观澜，失败后冷却 30 分钟。`openai.auth.revive.enabled`
是服务总闸，默认 true（不等于逐账号授权）；显式设为 false 后启动不注册该任务，界面提示服务停用。
签名原件和尝试状态保存在 runtime data 的 `revive/`，升级/备份须连同数据库保留该目录。原件哈希不符时
重新导入原始文件，不应修改签名或跳过验签。多账号签名文件不可截取重签，需全部授权且失效才执行。

回退到不支持观澜账号级开关的旧程序前，先关闭所有已勾选的开关并保存；关闭时不写新增凭据字段，旧程序才能读取。

若启用过「固定自身 state」实验开关，回退到不支持该功能的旧版本前，先在当前版本关闭相关账号的开关并保存。旧版凭据解析不认识新增配置字段；原始 state 只保存在进程内，重启后会重新捕获。
