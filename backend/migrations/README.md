...
生产库（89）在 2026-09-17 首次安装时，`0001`–`0011` 是以 **CRLF** 字节应用的；
`_sqlx_migrations` 里记录的正是这些字节的 checksum。因此本 fork 的这 11 个文件必须保持
CRLF 原样，`.frozen-sha256` 对应登记的是 CRLF 字节的哈希，与上游（LF）不同，**合并上游时
保留本 fork 的这 11 行**。`0012` 起与上游字节一致（LF）。根目录 `.gitattributes` 把
`*.sql` 标为 `-text`，任何平台检出都不做换行转换；不要去掉这条规则。
