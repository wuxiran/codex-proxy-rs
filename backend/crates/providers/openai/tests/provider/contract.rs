...
async fn turn_state_pin_websocket_metadata_is_replayed_in_next_turn_payload() {
    ...
    let server = tokio::spawn(async move {
        ...
        for round in 0..2 {
            ...
            for event in [
                json!({"type":"response.created","response":{"id":format!("resp_pin_ws_{round}"),"model":"gpt-5.4"}}),
                json!({"type":"response.metadata","headers":{"x-codex-turn-state": if round==0 {candidate.clone()} else {"b".repeat(312)}}}),
                json!({"type":"response.completed","response":{"id":format!("resp_pin_ws_{round}"),"model":"gpt-5.4","status":"completed","output":[],"usage":{"input_tokens":1,"output_tokens":1,"total_tokens":2}}}),
            ] {
                websocket
                    .send(Message::Text(event.to_string().into()))
                    .await
