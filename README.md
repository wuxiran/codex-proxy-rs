...
观澜 CDK 或原始签名 JSON 导入后，账号编辑页显示观澜来源及复活资格。勾选「401 后自动复活」并保存，
凭据失效时会自动申请观澜恢复，成功后写回匹配账号；页面可查看恢复状态与失败原因。默认不勾选，
额度耗尽、限流和普通网络错误不触发。批量签名原件不可拆签，其账号需全部勾选且已失效才自动提交。

OpenAI OAuth 账号的「账号管理 → 编辑账号 → 固定自身 state（实验）」可启用候选固定或重新捕获，查看当前模型、到期时间和命中次数。「最近捕获」读取近 24 小时最近 10 条请求的 state 长度摘要、结果和筛选说明。默认关闭；行为与效果边界见 [账号 API](docs/api.md#5-账号)。
